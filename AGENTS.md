...
- Project version **1.1.0** (`mix.exs` / `CHANGELOG.md`). Keep the changelog cut when work lands — do not park merged work under a stale Unreleased section. Full corpus→DB sync: `mix isomer.db.sync` (or `--dry-run`). Upserts `domain`, `framework`, `requirement`, `mapping_set`, `ruleset`, `rubric`, `question_set`, `template`; writes `maps_to` graph edges; prunes stale `content_source="repo"` rows; writes a `sync_run` record.
