...
- Guidance prefs on `user` (`self_role`, `experience_level`, `comfort_level`) via `/settings` when Surreal has those columns; otherwise prefs persist in the Phoenix session. **Generated documents (supported path):** `answer` rows with `pack_ref=__artifacts__` via `Isomer.Db.Tenant` — not the standalone `artifact` table (that table is DEFINE-only / forward readiness; Surreal Cloud multi-IP lag made app writes to `artifact` unreliable on Fly). `Isomer.GuideCopy` adapts wizard/library wording only — same UX for every level. Re-run `mix isomer.db.ensure_runtime` after pulling schema changes.
