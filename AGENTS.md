...
- Guidance prefs on `user` (`self_role`, `experience_level`, `comfort_level`) via `/settings` (Surreal SoR; cookie session is a non-nil overlay only). Reads use `SELECT … FROM ONLY $auth`. Failed Surreal writes surface an error flash — do not treat the success toast as “session-only is enough”. **Generated documents (supported path):** `answer` rows with `pack_ref=__artifacts__` via `Isomer.Db.Tenant` — not the standalone `artifact` table (that table is DEFINE-only / forward readiness; Surreal Cloud multi-IP lag made app writes to `artifact` unreliable on Fly). `Isomer.GuideCopy` adapts wizard/library wording only — same UX for every level. Re-run `mix isomer.db.ensure_runtime` after pulling schema changes.
