defmodule IsomerWeb.OrgLive.Show do
  ...
  def mount(%{"org_id" => org_id}, _session, socket) do
    ...
    case load_state(socket.assigns.surreal, org_id) do
      {:ok, state} ->
        {:ok, assign(socket, Map.merge(state, %{error: nil}))}
    ...
    <section class="isomer-page">
      <div class="isomer-page-header">
        ...
        <div class="flex flex-wrap gap-2">
          <.button
            :if={Roles.can_manage_members?(@current_role)}
            link_type="live_redirect"
            to={~p"/orgs/#{@org_id}/members"}
            color="gray"
            variant="outline"
            label="Members"
      ...
      <details
        open={true}
        class="group mb-4 rounded-lg border border-slate-200 open:pb-0 dark:border-slate-700"
      >
