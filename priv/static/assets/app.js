...
`,
  ...
  // ../deps/petal_components/assets/js/petal_components.js
  var PetalChatStream = {
    mounted() {
      ...
      this.handleEvent(event, (payload) => {
        ...
        const atEdge = this.scroller && this.scroller.scrollHeight - this.scroller.scrollTop - this.scroller.clientHeight < 80;
        this.el.dataset.started = "";
        if (payload.html !== void 0 && this.htmlEl) {
          this.htmlEl.innerHTML = payload.html;
        } else if (payload.text !== void 0 && this.textEl) {
  ...
  var PetalChatScroll = {
    mounted() {
      ...
      this.observer = new MutationObserver(() => {
        const shifted = this.anchor && this.anchor.isConnected ? this.anchor.offsetTop - this.anchorOffset : 0;
        if (shifted !== 0) {
    ...
    recordEdge() {
      this.wasAtEdge = this.el.scrollHeight - this.el.scrollTop - this.el.clientHeight < 80;
    },
    recordAnchor() {
      const top2 = this.el.scrollTop;
      const visible = [...this.el.children].filter(
        (c) => c.offsetTop + c.offsetHeight > top2
      );
  ...
  var PetalDualRangeSlider = {
    ...
    syncTrack(min, max) {
      min = min !== void 0 ? min : parseFloat(this.minInput.value);
      max = max !== void 0 ? max : parseFloat(this.maxInput.value);
  ...
  var PetalChart = {
    mounted() {
      if (!window.echarts) {
        console.warn(
          '[petal] PetalChart: window.echarts not found. Add ECharts to your app (e.g. <script src="https://cdn.jsdelivr.net/npm/echarts@5/dist/echarts.min.js"><\/script>).'
        );
        return;
      }
    ...
    syncLoading() {
      if (this.el.dataset.loading === "true") {
        ...
        this.chart.showLoading("default", {
          ...
          maskColor: dark ? this.alpha("var(--color-gray-900)", 60) : "rgba(255, 255, 255, 0.6)",
          spinnerRadius: 12,
          lineWidth: 3
        });
      } else {
    ...
    prepareOption(opt) {
      ...
      series.forEach((s, i) => {
        ...
        const base = this.normalizeColor(
          s.color || s.itemStyle && s.itemStyle.color || ""
        ) || palette[i % palette.length];
        const stop = (alphaPct) => base.replace(
          /rgba\(([^)]+),\s*[\d.]+\)/,
          `rgba($1, ${alphaPct / 100})`
        );
        s.areaStyle = {
    ...
    formatterFor(spec) {
      const m = /^petal:(number|percent|currency|currency-compact)(?::([A-Za-z]{3}))?$/.exec(
        spec
      );
      if (!m) return null;
      const wrap = (nf, suffix) => (value) => typeof value === "number" ? nf.format(value) + (suffix || "") : value;
      switch (m[1]) {
        case "number":
          return wrap(
        ...
        case "percent":
          return wrap(
            new Intl.NumberFormat(void 0, { maximumFractionDigits: 1 }),
            "%"
          );
        case "currency":
          return wrap(
    ...
    palette() {
      ...
      const candidates = [
        ...
        "success",
        "secondary"
      ].map((role) => this.resolveColor(`var(--color-${role}-500)`)).filter(Boolean);
      const picked = [];
      const demoted = [];
      for (const color of candidates) {
        const h = this.hueOf(color);
        const clashes = h !== null && picked.some((p) => {
    ...
    buildTheme() {
      ...
      return {
        ...
        tooltip: {
          ...
          padding: [8, 12],
          extraCssText: "border-radius: clamp(0.25rem, calc(var(--pc-radius, 0.625rem) - 0.125rem), 0.875rem); box-shadow: 0 4px 14px rgba(0, 0, 0, 0.18);"
        }
      };
    }
  };
  var PetalColorScheme = {
    mounted() {
  ...
  var PetalNumberTicker = {
    ...
    updated() {
      ...
      if (target !== this.lastTarget) {
        const from = this.current !== void 0 ? this.current : this.lastTarget;
        this.lastTarget = target;
        this.animate(from);
      } else if (this.current !== void 0) {
    ...
    render(value) {
      ...
      const fmt = new Intl.NumberFormat(this.el.dataset.locale || void 0, {
        minimumFractionDigits: decimals,
        maximumFractionDigits: decimals
      });
      this.el.textContent = (this.el.dataset.prefix || "") + fmt.format(value) + (this.el.dataset.suffix || "");
    }
  };
  var PetalConfetti = {
    defaultColors: [
    ...
    fire(opts) {
      ...
      const angle = opts.angle !== void 0 ? opts.angle : 90;
  ...
  if (typeof window !== "undefined" && !window.__petalComponentsAccordionInit) {
    ...
    window.addEventListener("click_accordion", (e) => {
      ...
      const container = document.getElementById(e.detail.container_id) || (clickedAccordionItem.closest("[data-i]") || {}).parentElement;
      if (!container) return;
  ...
  var PetalInputOTP = {
    ...
    sanitize() {
      const pattern = this.el.dataset.pattern === "alphanumeric" ? /[^a-zA-Z0-9]/g : /[^0-9]/g;
  ...
  var PetalPopover = {
    ...
    position() {
      ...
      let s = side;
      if (side === "bottom" && space.bottom < p.height + gap && space.top > space.bottom)
        s = "top";
      if (side === "top" && space.top < p.height + gap && space.bottom > space.top)
        s = "bottom";
      if (side === "right" && space.right < p.width + gap && space.left > space.right)
        s = "left";
      if (side === "left" && space.left < p.width + gap && space.right > space.left)
        s = "right";
      let top2, left;
      if (s === "top" || s === "bottom") {
        top2 = s === "top" ? t.top - p.height - gap : t.bottom + gap;
        ...
        if (align === "start") top2 = t.top;
        else if (align === "end") top2 = t.bottom - p.height;
        else top2 = t.top + t.height / 2 - p.height / 2;
      }
      left = Math.max(pad, Math.min(left, window.innerWidth - p.width - pad));
      top2 = Math.max(pad, Math.min(top2, window.innerHeight - p.height - pad));
  ...
  var PetalNavMenu = {
    mounted() {
      ...
      this.onResize = () => {
        const open = this.items.find(
          (i) => i.classList.contains("pc-nav-menu__item--open")
        );
    ...
    closeAll() {
      clearTimeout(this.closeTimer);
      this.items.forEach(
        (item) => this.close(item, item.querySelector(".pc-nav-menu__trigger"))
      );
    },
  ...
  var PetalLocalTime = {
    ...
    render() {
      ...
      if (cfg.format === "relative") {
        const age = (Date.now() - date.getTime()) / 1e3;
        ...
        this.timer = setTimeout(this.render, next * 1e3);
      } else {
  ...
  var PetalCarousel = {
    mounted() {
      ...
      this.autoplayInterval = parseInt(this.el.dataset.autoplayInterval) || 5e3;
      ...
      this.swipe = this.el.dataset.swipe !== "false";
    ...
    cloneSlide(slideToClone) {
      const clone2 = slideToClone.cloneNode(true);
      clone2.setAttribute("aria-hidden", "true");
      clone2.removeAttribute("id");
      clone2.querySelectorAll("[id]").forEach((n) => n.removeAttribute("id"));
      this.applySlideDimensions(clone2);
      return clone2;
    },
    ...
    initScrollSnapCarousel() {
      ...
      this.slideWrapper.style.scrollbarWidth = "none";
    ...
    initFadeCarousel() {
      this.transitionDuration = parseInt(this.el.dataset.transitionDuration) || 500;
    ...
    forward() {
      ...
      this.updateButtonStates();
      setTimeout(() => {
        this.goto(this.n_slides - 1, false);
      }, 50);
    },
    updateSlideWidth() {
      if (this.slideWrapper) {
        const containerSize = this.isVertical ? this.carouselContainer.offsetHeight : this.carouselContainer.offsetWidth;
        ...
        if (this.slides.length > 1) {
          setTimeout(() => {
            ...
            const actualSpacing = this.isVertical ? this.slides[1].offsetTop - this.slides[0].offsetTop : this.slides[1].offsetLeft - this.slides[0].offsetLeft;
    ...
    setupMouseDrag() {
      ...
      const handleMouseDown = (e) => {
        ...
        this.slideWrapper.style.userSelect = "none";
        this.slideWrapper.style.scrollSnapType = "none";
        if (this.isVertical) {
    ...
    setupKeyboardNavigation() {
      this.keyboardHandler = (e) => {
        ...
        e.preventDefault();
        const isNext = this.isVertical ? e.key === "ArrowDown" : e.key === "ArrowRight";
        const isPrev = this.isVertical ? e.key === "ArrowUp" : e.key === "ArrowLeft";
        if (isNext) {
          this.nextSlide();
        } else if (isPrev) {
          this.prevSlide();
    ...
    updateButtonStates() {
      if (this.loop) {
        const prevButton2 = this.wrapper.querySelector(
          `#${this.id}-carousel-prev`
        );
        const nextButton2 = this.wrapper.querySelector(
          `#${this.id}-carousel-next`
        );
        if (prevButton2) prevButton2.disabled = false;
  ...
  var toastGroups = /* @__PURE__ */ new Set();
  var isPrimaryToastGroup = (hook) => toastGroups.values().next().value === hook;
  var PetalToast = {
    mounted() {
      ...
      if (!isPrimaryToastGroup(this)) {
        ...
        console.warn(
          `[petal_components] More than one <.toast_group> is mounted: #${owner.el.id} and #${this.el.id}. Toasts are global, so a second group would render an identical toast behind every one - #${this.el.id} will ignore server and window toast events. Keep a single group in your layout. (Which group wins is not guaranteed to follow DOM order, so remove the extra rather than relying on this.)`
        );
      }
      ...
      this.defaultDuration = parseInt(this.el.dataset.duration) || 5e3;
      this.toasts = [];
      this.seq = 0;
      this.expanded = false;
      ...
      this.handleEvent("petal:toast-dismiss", (d) => {
        ...
        if (d && d.all) [...this.toasts].forEach((t) => this.dismiss(t));
          ...
          const t = this.toasts.find((t2) => t2.id === String(d.id));
          if (t) this.dismiss(t);
        }
      });
      ...
      this.onClick = (e) => {
        ...
        if (!toastEl) return;
        const t = this.toasts.find((t2) => t2.el === toastEl);
        if (!t) return;
    ...
    contentFor(t) {
      const action = t.action ? '<button type="button" class="pc-toast__action" data-toast-action="' + this.esc(t.action.event || "") + `" data-toast-value='` + this.esc(JSON.stringify(t.action.value || {})) + "'>" + this.esc(t.action.label) + "</button>" : "";
      const close = t.closeable ? '<button type="button" class="pc-toast__close" data-toast-close aria-label="Dismiss"><span class="hero-x-mark pc-toast__close-icon" aria-hidden="true"></span></button>' : "";
      const desc = t.description ? '<p class="pc-toast__description">' + this.esc(t.description) + "</p>" : "";
      const progress = t.duration > 0 && t.progress ? '<div class="pc-toast__progress pc-toast__progress--' + this.esc(t.kind) + '" style="animation-duration: ' + t.duration + 'ms"></div>' : "";
      return '<div class="pc-toast__body">' + this.iconFor(t.kind) + '<div class="pc-toast__text"><div class="pc-toast__title">' + this.esc(t.title) + "</div>" + desc + "</div>" + action + close + "</div>" + progress;
    },
    // ----------------------------------------------------------------- upsert
    upsert(d) {
      const id = d.id != null ? String(d.id) : "pc-toast-" + ++this.seq;
      const kind = d.kind || "neutral";
      const duration = d.duration != null ? d.duration : kind === "loading" ? 0 : this.defaultDuration;
      let t = this.toasts.find((t2) => t2.id === id);
      if (t) {
  ...
  var PetalComboBox = {
    mounted() {
      ...
      this.onControlClick = (e) => {
        ...
        for (const [id, v] of this.pressVerdicts) {
          if (now - v.at > 1e3) this.pressVerdicts.delete(id);
        }
        let chrome;
        ...
        if (this.panel.hidden) {
          if (now - this.suppressOpenAt < 1e3) return;
          this.openPanel();
          this.input.focus();
        } else if (chrome) {
          this.closePanel();
      ...
      this.onOutsidePointerDown = (e) => {
        ...
        this.outsidePress = this.el.contains(e.target) ? null : { id: e.pointerId, x: e.clientX, y: e.clientY };
      };
      ...
      this.pressVerdicts = /* @__PURE__ */ new Map();
      this.suppressOpenAt = -Infinity;
      if (this.control) {
    ...
    openPanel({ keepQuery = false } = {}) {
      ...
      if (this.trigger) this.trigger.setAttribute("aria-expanded", "true");
      this.outsidePress = null;
      this.activePointers = /* @__PURE__ */ new Set();
    ...
    selectedValues() {
      return Array.from(this.select.selectedOptions).map((o) => o.value).filter((v) => v !== "");
    },
    maxReached() {
    ...
    syncChips() {
      ...
      const want = Array.from(this.select.selectedOptions).map((o) => o.value).filter((v) => v !== "");
    ...
    applyChipOrder() {
      ...
      if (seq.length === current.length && seq.every((c, i) => c === current[i])) {
        return;
      }
      ...
      if (this.triggerLabel && this.triggerLabel.dataset.customLabel != null) {
        ...
        if (Array.isArray(rendered) && sameValueMultiset(rendered, values)) {
          ...
          if (form && form.hasAttribute("phx-change") && values.length > 0) {
            ...
            if (performance.now() - this.labelStaleSince < 2e3) {
              labelIsFresh = true;
            }
          }
        }
    ...
    remoteSearch() {
      if (typeof this.pushEventTo !== "function" && typeof this.pushEvent !== "function") {
        return;
      }
    ...
    renderRemoteResults(results) {
      ...
      const anchor = (this.createRow && this.createRow.parentElement === this.list ? this.createRow : null) || this.el.querySelector("[data-pc-combo-empty]");
    ...
    filter() {
      ...
      if (this.createRow) {
        const q = query.trim();
        const exact = q && this.items().some(
          (i) => (i.dataset.label || "").trim().toLowerCase() === q.toLowerCase()
        );
  ...
  var StickyTitle = {
    mounted() {
      ...
      const title = this.el.dataset.stickyTitle || this.el.textContent.trim();
      this.observer = new IntersectionObserver(
        ([entry]) => {
          if (!this.target) return;
  ...
  var liveSocket = new LiveSocket("/live", Socket, {
    ...
    hooks: { ...petal_components_default, StickyTitle }
  });
  liveSocket.connect();
